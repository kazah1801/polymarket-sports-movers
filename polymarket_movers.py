...
class PolymarketSportsMovers:
    ...
    def refresh_universe(self) -> None:
        ...
        for event in events:
            ...
            event_sources = self._event_sources(event)
    ...
    def poll_best_asks(self) -> None:
        ...
        prices = self._normalize_market_quotes(prices)
...
def _resolve_target_tag_ids(
    ...
    def _fetch_price_quote_from_book(self, asset_id: str) -> Tuple[Optional[PriceQuote], Optional[str]]:
        ...
        for path, params in endpoints:
            ...
            if not isinstance(asks, list) or not asks or not isinstance(bids, list) or not bids:
                ...
                logging.info(
                    "Orderbook missing side: path=%s | asset_id=%s | bids=%s | asks=%s",
                    path,
                    asset_id,
                    bids_count,
                    asks_count,
            ...
            if best_bid is None or best_ask is None:
                last_empty_reason = f"path={path} invalid_top_levels"
                continue
            if best_bid <= 0 or best_ask <= 0 or best_bid > 1 or best_ask > 1 or best_bid > best_ask:
                last_empty_reason = f"path={path} invalid_prices"
                continue
            spread_pp = (best_ask - best_bid) * 100.0
            if spread_pp > MAX_SPREAD_PP:
                logging.info(
                    "Wide spread skipped: asset_id=%s | best_bid=%.4f | best_ask=%.4f | spread_pp=%.2f",
                    asset_id,
                    best_bid,
                    best_ask,
                    spread_pp,
                )
                return None, "rejected_wide_spread"
    ...
    def _primary_moneyline_reject_reason(
        ...
        yes_no_only = unique_outcomes.issubset({"yes", "no"})
        winner_hints = ("moneyline", "match winner", "winner", "to win", "win in regulation", "game winner")
        has_winner_hint = any(hint in title for hint in winner_hints)
        if yes_no_only:
            return "non_moneyline"

        non_empty_unique = {item for item in unique_outcomes if item}
        normalized_outcomes = {self._normalize_market_outcome_name(item) for item in non_empty_unique}
        has_draw = any(item in {"draw", "tie", "x"} for item in normalized_outcomes)
        has_home_away = {"home", "away"}.issubset(normalized_outcomes)
        has_named_sides = bool(non_empty_unique) and not (normalized_outcomes & {"yes", "no"})
        ...
        if len(raw_outcomes) == 2 and has_named_sides and not yes_no_only and (has_matchup_hint or has_home_away or has_winner_hint):
            return None
        if sport == "football" and len(raw_outcomes) == 3 and has_draw and len(normalized_outcomes) == 3:
            return None
        if has_home_away and len(raw_outcomes) in {2, 3} and has_winner_hint:
            return None
        if has_matchup_hint and has_named_sides:
            return None
        return "non_moneyline"
    ...
    def _is_supported_outcome(self, outcome_name: str, sport: str) -> bool:
        ...
        return bool(name) and name not in {"yes", "no"} and not any(term in name for term in PROP_MARKET_TERMS)
    ...
    def _normalize_market_quotes(self, prices: Dict[str, PriceQuote]) -> Dict[str, PriceQuote]:
        if not prices:
            logging.info("Price normalization: markets=0 | outcomes=0")
            return prices
        ...
        normalized_outcome_count = 0
        for market_id, items in grouped.items():
            if len(items) not in {2, 3}:
                continue
            total_midpoint = sum(max(quote.midpoint, 0.0001) for _, quote in items)
            if total_midpoint <= 0:
                continue
            if abs(total_midpoint - 1.0) <= NORMALIZATION_EPSILON:
                continue
            normalized_market_count += 1
            for ref, quote in items:
                normalized_midpoint = min(max(quote.midpoint / total_midpoint, 0.0001), 0.9999)
                half_spread = min(quote.spread_pp / 200.0, 0.05)
                bid = max(0.0001, normalized_midpoint - half_spread)
                ask = min(0.9999, normalized_midpoint + half_spread)
                if bid > ask:
                    bid = normalized_midpoint
                    ask = normalized_midpoint
                normalized[ref.asset_id] = PriceQuote(
                    bid=bid,
                    ask=ask,
                    midpoint=normalized_midpoint,
                    spread_pp=max(0.0, (ask - bid) * 100.0),
                    source=(
                        f"{quote.source}|market_normalized("
                        f"market_id={market_id},raw_sum={total_midpoint:.4f},"
                        f"normalized_price={normalized_midpoint:.4f})"
                    ),
                )
                normalized_outcome_count += 1

        logging.info(
            "Price normalization: markets=%s | outcomes=%s | epsilon=%s",
            normalized_market_count,
            normalized_outcome_count,
    ...
    def _log_source_pipeline(self, source_counts: Dict[str, Dict[str, int]]) -> None:
        ...
        for source, counts in sorted(source_counts.items()):
            logging.info(
                "Universe source pipeline: source=%s | raw=%s | deduped=%s | with_markets=%s | after_date_window=%s | after_exclude_live_ended_started_closed=%s | with_candidate_market=%s | after_moneyline=%s",
                source,
                counts.get("raw", 0),
    ...
    def _infer_sport_from_league(league: str) -> str:
        ...
        if "mlb" in normalized:
            return "mlb"
        if "ufc" in normalized or "mma" in normalized:
            return "mma"
    ...
    def _log_tracked_outcomes(self) -> None:
        if not self.outcomes:
            ...
            return
        self._log_tracked_refs(list(self.outcomes.values())[:50])
    ...
    def _log_monitoring_outcomes(
        ...
        if monitored_refs:
            self._log_tracked_refs(monitored_refs[:50])
        else:
            logging.info("TRACKED none")
    ...
    def _log_tracked_market_summary(self) -> None:
        ...
        for idx, refs in enumerate(list(grouped.values())[:30], start=1):
            first = refs[0]
    ...
    def _filter_market_ids_by_high_prob(self, prices: Dict[str, PriceQuote]) -> Tuple[set[str], List[MarketFilterHit]]:
        ...
        for market_id, items in grouped.items():
            ...
            for ref, quote in items:
                if quote.midpoint > HIGH_PROB_THRESHOLD:
                    offending = (ref, quote)
                    break
    ...
    def _log_filtered_markets_gt_90(self, hits: List[MarketFilterHit]) -> None:
        ...
        for hit in hits[:20]:
            logging.info(
                "MARKET_SKIPPED_GT_90 | event=%s | market=%s | outcomes=%s | prices=%s | offending_outcome=%s | offending_price=%.4f | sport=%s | league=%s | slug=%s",
    ...
    def _log_top_movers(self, movers: List[Tuple[float, OutcomeRef, float, Optional[float]]]) -> None:
        ...
        for idx, (delta_pp, ref, price, prev_price) in enumerate(sorted(movers, key=lambda item: item[0], reverse=True)[:10], start=1):
            logging.info(
                "MOVER %02d | event=%s | outcome=%s | price=%.4f | prev=%s | delta_pp=%.2f",
                idx,
                self._log_field(ref.event_title or "-"),
                self._log_field(ref.outcome_name or "-"),
                price,
...
def load_config() -> Config:
    ...
    horizon_days = env_int("HORIZON_DAYS", 5)
