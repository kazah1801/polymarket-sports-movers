...
class PolymarketSportsMovers:
    ...
    def poll_best_asks(self) -> None:
        ...
        alerts_blocked_best_ask_sum = 0
        market_best_ask_sums = self._compute_market_best_ask_sums(prices)
        ...
        for key, ref in self.outcomes.items():
            ...
            market_best_ask_sum = market_best_ask_sums.get(ref.market_id)
            if market_best_ask_sum is None or market_best_ask_sum > self.config.best_ask_sum_max:
                alerts_blocked_best_ask_sum += 1
                continue
            ...
            self.notifier.send(self._format_alert(ref, old_price, current_price, move_pp, direction, market_best_ask_sum))
            alerts_triggered += 1
        ...
        logging.info(
            "Alert filter best_ask_sum: max=%.4f | markets_with_complete_prices=%s | blocked_alerts=%s",
            self.config.best_ask_sum_max,
            len(market_best_ask_sums),
            alerts_blocked_best_ask_sum,
        )
    ...
    def _fetch_sports_universe(self) -> Tuple[List[Dict[str, Any]], Dict[str, str], int, Dict[str, Dict[str, int]]]:
        ...
        if "football" in ALLOWED_SPORTS and not any(source == "sports:football" or source.startswith("tags:") and self._infer_sport_from_league(source.split(":", 1)[1]) == "football" for source in tag_sources.values()):
            logging.warning("Футбольные source tags не определились: нет sports:football и нет football league tags")
    ...
    def _resolve_target_tag_ids(
        ...
        for item in sports_metadata:
            ...
            for normalized_sport, aliases in SPORTS_METADATA_SPORTS.items():
                if sport_name not in aliases:
                    continue
                source_name = normalized_sport if normalized_sport == "football" else sport_name
                for tag_id in self._csv_list(item.get("tags")):
                    resolved[tag_id] = f"sports:{source_name}"
                break
        ...
        for tag in tags:
            ...
            if any(keyword in haystack for keyword in FOOTBALL_GENERIC_TAG_KEYWORDS):
                resolved[tag_id] = "sports:football"
                continue
    ...
    def _compute_market_best_ask_sums(self, prices: Dict[str, PriceQuote]) -> Dict[str, float]:
        ...
        for market_id, items in grouped.items():
            if len(items) != expected_counts.get(market_id, 0):
                continue
            sums[market_id] = sum(self._monitored_price(quote) for _ref, quote in items)
        return sums
    ...
    def _format_alert(
        ...
        move_pp: float,
        direction: str,
        market_best_ask_sum: Optional[float] = None,
    ) -> str:
        best_ask_sum_line = ""
        if market_best_ask_sum is not None:
            best_ask_sum_line = f"Сумма best ask по рынку: {market_best_ask_sum:.4f}\n"
        return (
...
def load_config() -> Config:
    ...
    return Config(
        ...
        best_ask_sum_max=float(env_str("BEST_ASK_SUM_MAX", "1.07")),
    )
